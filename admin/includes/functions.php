...
function login()
{
    if(isset($_POST['login'])) {
        ...
        $query = "SELECT admin_id, admin_email, admin_password FROM admin WHERE admin_email='$email'";
        $data = query($query);
...
function edit_item($id) {
    if (isset($_POST['update'])) {
        ...
        // Handle file upload
        $image = $_POST['current_image']; // Use existing image by default
        ...
        if ($check == 0) {
            $query = "UPDATE item SET item_title='$name', item_brand='$brand', item_cat='$cat', item_details='$details', item_tags='$tags', item_image='$image', item_quantity='$quantity', item_price='$price' WHERE item_id='$id'";
            $run = single_query($query);
            get_redirect("products.php");
        } else {
...
function add_item() {
    if (isset($_POST['add_item'])) {
        ...
        $check = check_name($name);
        if (empty($name) || empty($brand) || empty($cat) || empty($tags) || empty($image) || empty($quantity) || empty($price) || empty($details)) {
            $_SESSION['message'] = "empty_err";
            get_redirect("products.php");
            return;
        }
        if ($check == 0) {
            $query = "INSERT INTO item (item_title, item_brand, item_cat, item_details, item_tags, item_image, item_quantity, item_price) VALUES ('$name', '$brand', '$cat', '$details', '$tags', '$image', '$quantity', '$price')";
            $run = single_query($query);
            get_redirect("products.php");
        } else {
            $_SESSION['message'] = "itemErr";
